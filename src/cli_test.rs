...
#[test]
fn zero_oid_diff_range_exits_zero() {
    let repo = TestRepo::new();
    repo.write("hello.rs", "fn main() {}\n");
    repo.commit("initial");

    repo.run()
        .args(["--diff", "0000000000000000000000000000000000000000..HEAD"])
        .assert()
        .success();
}
...
#[test]
fn diff_detects_stale_file_reference_after_pure_rename() {
    let repo = TestRepo::new();
    repo.write(
        "ref.rs",
        "
    ...
    repo.git(&["mv", "b.rs", "c.rs"]);
    repo.commit("rename target");
    ...
    let stderr = String::from_utf8_lossy(&result.get_output().stderr);
    assert!(
        stderr.contains("ref.rs:1"),
        "expected stale-reference finding in ref.rs, got: {stderr}",
    );
    assert!(
        stderr.contains("target file not found: b.rs"),
        "expected stale old-path finding after pure rename, got: {stderr}",
    );
}

#[test]
fn diff_detects_stale_file_reference_after_rename_with_edit() {
    let repo = TestRepo::new();
    repo.write(
        "ref.rs",
        "
    ...
    repo.git(&["mv", "b.rs", "c.rs"]);
    repo.write("c.rs", "new_b\n");
    repo.commit("rename and edit target");
    ...
    assert!(
        stderr.contains("target file not found: b.rs"),
        "expected stale old-path finding after rename with edit, got: {stderr}",
    );
}

#[test]
fn diff_detects_multiple_deleted_targets() {
    let repo = TestRepo::new();
    repo.write(
        "a.rs",
        "
    ...
    repo.remove("x.rs");
    repo.remove("y.rs");
    repo.commit("delete both targets");
    ...
    let stderr = String::from_utf8_lossy(&result.get_output().stderr);
    assert!(
        stderr.contains("a.rs:1"),
        "expected stale-reference finding for a.rs, got: {stderr}",
    );
    assert!(
        stderr.contains("b.rs:1"),
        "expected stale-reference finding for b.rs, got: {stderr}",
    );
    assert!(
        stderr.contains("target file not found: x.rs"),
        "expected missing x.rs finding, got: {stderr}",
    );
    assert!(
        stderr.contains("target file not found: y.rs"),
        "expected missing y.rs finding, got: {stderr}",
    );
}
...
#[test]
fn outside_root_relative_file_is_skipped() {
    let repo = TestRepo::new();
    let mut outside = Builder::new()
        .suffix(".rs")
        .tempfile_in(repo.dir.path().parent().unwrap())
        .unwrap();
    write!(
        outside,
        "{}",
    ...
    let result = repo.run().arg(&rel).assert().success();
    let stderr = String::from_utf8_lossy(&result.get_output().stderr);
    assert!(
