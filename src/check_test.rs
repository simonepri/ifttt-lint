...
#[parameterized(
    ...
    // Only the FIRST block comment on a line is extracted. A directive that
    // appears in a second block comment on the same line is silently skipped.
    // This avoids extra scanning complexity; directives are expected on their
    // own dedicated comment lines.
    second_block_comment_on_same_line_ignored = { CheckCase {
        files: files!{
            "test.js" => "
                /* preamble */ /* LINT.ThenChange(//missing.rs) */
                real();
            ",
        },
        changes: Some(const { &[sub("test.js", &[1, 2])] }),
        // No ThenChange without IfChange error — the directive is never seen.
    ...
    // A directive following a block-comment close on the same line is
    // intentionally ignored. Directives must appear alone on their own
    // comment line — the scanner does not resume scanning past a close marker.
    directive_after_block_comment_close_mid_line_ignored = { CheckCase {
        files: files!{
            "test.c" => "
                /* block comment
                   spanning multiple lines */ // LINT.ThenChange(//missing.c)
                real();
            ",
        },
        changes: Some(const { &[sub("test.c", &[1, 2, 3])] }),
        // ThenChange is never seen, so no 'without preceding IfChange' error.
        expected_errors: Some(&[]),
        expected_findings: Some(&[]),
        ..DEFAULTS
    } },
...
",
    ...
    block_comment_opener_in_string_not_confused = { CheckCase {
        // A block-comment opener inside a string literal must not switch the
        // scanner into InBlockComment state.
        files: files! {
            "test.c" => r#"
                char *s = "/*";
                // LINT.IfChange
        ...
        changes: Some(const { &[sub("test.c", &[3])] }),
        expected_findings: Some(&["may need to be reflected in"]),
        expected_errors: Some(&[]),
        ..DEFAULTS
    } },
    ...
    multiline_then_change_non_consecutive_lines_malformed = { CheckCase {
        // A blank non-comment line between LINT.ThenChange( and its continuation
        // breaks token consecutiveness; collect_multiline stops early and the
        // directive is reported as malformed.
        files: files! {
            "a.toml" => "
                # LINT.IfChange
                version = \"1.0\"
    ...
    inline_directive_after_code_ignored = { CheckCase {
        // A LINT directive in a trailing inline comment (code before //) is
        // ignored — directives must appear alone on their comment line.
        // scan_normal only matches when the trimmed line starts with the
        // comment prefix, so code(); // LINT.IfChange produces no token.
        files: files! {
            "test.rs" => "
                int x = 1; // LINT.IfChange
                code
                int y = 2; // LINT.ThenChange(//b.rs)
            ",
            "b.rs" => "x\n",
        },
        expected_findings: Some(&[]),
...
// Files discovered only via the reverse-lookup pass (not in the diff and not in
// validate_files) are unmodified bystanders. Their stale-reference findings ARE
// reported, but their own parse errors are intentionally suppressed to avoid
// noise from files the user never touched.
#[test]
fn reverse_lookup_parse_errors_suppressed() {
    let mut mock = MockVcsProvider::default();
    // a.rs has a valid pair pointing to deleted b.rs, plus an unknown directive
    // that the parser would normally flag as an error.
    mock.add_file(
        "a.rs",
    ...
    assert!(
        result
            .iter()
            .any(|d| d.message.contains("target file not found: b.rs")),
        "stale reference to deleted b.rs must be reported; got: {result:?}",
    );
    assert!(
        !result
            .iter()
            .any(|d| d.message.contains("unknown directive")),
        "parse errors in reverse-lookup-only files must not be reported; got: {result:?}",
    );
}
...
#[parameterized(
    ...
    backslash_path_with_directory_stays_root_relative = { CheckCase {
        // Backslashes are normalized before resolution, so a nested path
        // must stay root-relative rather than being treated like a bare filename.
        files: files! {
            "a/b/foo.h" => "
                // LINT.IfChange
                int x;
...
// A file with NO IfChange directives that has only additions in the diff still
// participates in the reverse-lookup pass (build_label_sets includes it with an
// empty label set).  Any surviving file referencing it with a label should get
// a stale-label error, just as in the has-IfChange cases above.
#[test]
fn stale_label_reverse_lookup_no_ifchange_file() {
    run_case(&CheckCase {
        files: files! {
            // a.rs has no IfChange directives at all.
            "a.rs" => "
                fn a() {}
            ",
            // ref.rs references a non-existent label in a.rs.
            "ref.rs" => "
                // LINT.IfChange
                code
                // LINT.ThenChange(//a.rs:nonexistent_label)
            ",
        },
        // a.rs is in the diff with only additions (no removals).
        changes: Some(&[("a.rs", &[1], &[])]),
        file_list: &[],
        // The reverse lookup must catch the stale label from ref.rs.
        expected_findings: Some(&[]),
        expected_errors: Some(&["label nonexistent_label not found in a.rs"]),
        ..DEFAULTS
    });
}
