...
fn run_check(
    ...
    run_validation(&scope, &ctx, &mut result);
...
/// target/label exist?).
fn run_validation(scope: &ValidationScope, ctx: &CheckContext<'_>, result: &mut CheckResult) {
    let diagnostics: Vec<CheckResult> = scope
        .validate_files
        .par_iter()
        .filter_map(|file_str| {
            let parsed = ctx.cache.get(file_str)?;
            let sorted = ctx.sorted_lines.get(file_str.as_str());
            let pairs = build_pairs(&parsed.directives);
            let mut file_result: CheckResult = Vec::new();
            for (pair, target, triggered) in
                active_targets(&pairs, sorted, file_str, ctx.vcs, ctx.ignore_patterns)
            {
...
/// When `--strict=false` allows bare filenames, resolve them relative to the
/// source file's directory. Paths that already contain a separator are left
...
/// have had labels renamed or removed. Uses owned strings to avoid borrowing
/// cache across the mutable pass.
///
...
///
/// Skips files where nothing was removed — labels are intact, and any new
/// IfChange directives are additions (not renames), so they can't produce
/// stale reverse references.
fn build_label_sets(
    changes: &ChangeMap,
    cache: &ParseCache,
    sorted_lines: &HashMap<&str, SortedLines>,
    ...
    changes
        ...
        .filter_map(|(path, _)| {
            ...
            if has_if_change {
                ...
                // If nothing was removed, no label was deleted or renamed — a
                // newly added IfChange can't create stale reverse references.
                if sorted.removed_new_pos.is_empty() {
                    return None;
                }
            }
