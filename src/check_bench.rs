...
fn init_repo(dir: &Path) {
    for args in [
        vec!["init", "-b", "main"],
        vec!["config", "user.email", "bench@example.com"],
        ...
        let status = std::process::Command::new("git")
            ...
            .status()
            .expect("git must be available for benchmarks");
...
/// Creates a real git repo with the given files and returns a GitVcsProvider.
/// All lines in every file are marked as added in the change map.
fn setup_git(files: &[(&str, &str)]) -> (TempDir, GitVcsProvider, ChangeMap) {
...
mod cpu_bound {
    ...
    fn large_file_pair(bencher: divan::Bencher, n_blocks: usize) {
        ...
        let (_dir, vcs, changes) = setup_git(&files);

        bencher.bench_local(|| {
            check::check(&vcs, &changes, &[], 0).unwrap();
        });
    }
}
...
mod deleted_target_reverse_lookup {
    ...
    fn repo_size(bencher: divan::Bencher, cfg: RepoSize) {
        ...
        for i in 0..n_plain {
            files.push((format!("plain_{i}.rs"), format!("fn plain_{i}() {{}}\n")));
        }
