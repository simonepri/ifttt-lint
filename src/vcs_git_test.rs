...
#[test]
fn diff_with_zero_oid_range_is_empty() {
    let dir = tempfile::tempdir().unwrap();
    init_repo(dir.path());
    ...
    assert!(
        changes.is_empty(),
        "zero-OID ranges should be treated as empty"
    );
}

#[test]
fn diff_with_pure_rename_marks_old_path_deleted() {
    let dir = tempfile::tempdir().unwrap();
    init_repo(dir.path());

    std::fs::write(dir.path().join("before.txt"), "same content\n").unwrap();
    git_add_commit(dir.path(), "baseline");
    ...
    git_add_commit(dir.path(), "rename file");
    ...
    let old = changes
        .get("before.txt")
        .expect("old path should appear in diff");
    assert!(old.deleted, "old path should be treated as deleted");

    let new = changes
        .get("after.txt")
        .expect("new path should appear in diff");
    assert!(!new.deleted, "new path should remain a live file");
}
...
fn diff_range_to_log_range_cases(input: &str, expected: &str) {
    assert_eq!(diff_range_to_log_range(input), expected);
}

#[parameterized(
    zero_base_two_dot   = { &format!("{NULL_OID}..HEAD"),  true  },
    zero_head_two_dot   = { &format!("HEAD..{NULL_OID}"),  true  },
    zero_base_three_dot = { &format!("{NULL_OID}...HEAD"), true  },
    normal_range        = { "main...HEAD",                false },
    single_ref          = { "HEAD",                       false },
)]
fn range_has_null_ref_cases(input: &str, expected: bool) {
    assert_eq!(range_has_null_ref(input), expected);
}
